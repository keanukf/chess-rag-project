...
def preprocess_data(filters: Dict[str, Any]) -> pd.DataFrame:
    df = load_chess_data()
    ...
    if "rated" in filters:
        df = rated_filter(df, filters["rated"])
...
def get_user_filters() -> Dict[str, Any]:
    ...
    rated_input = input("Filter for rated or not rated games? (rated/not rated) or press Enter to skip: ").lower()
    if rated_input == "rated":
        filters["rated"] = True
    elif rated_input == "not rated":
        filters["rated"] = False
